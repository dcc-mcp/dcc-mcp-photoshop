...
`photoshop-setup` 技能提供完整的一键安装工作流，可自动完成环境检查、包安装、UXP 插件安装和 MCP 客户端配置。
...
| 工具 | 说明 |
|------|------|
| `check_environment` | 检查系统环境（Python、pip、Photoshop） |
| `install_package` | 通过 pip 安装或升级 dcc-mcp-photoshop |
| `setup_uxp_plugin` | 安装 UXP .ccx 插件到 Photoshop |
...
1. **check_environment** — 检查 Python、pip 和 Photoshop 是否就绪。
2. **install_package** — 从 PyPI 安装 `dcc-mcp-photoshop` 及其依赖。
3. **setup_uxp_plugin** — 将 `.ccx` 插件安装到 Photoshop。
4. **configure_mcp_client** — 为 Claude Desktop、Cursor 和 VS Code 配置 MCP 连接。
5. **verify_connection** — 确认 bridge 连接正常，Photoshop 可响应。

### 安装指定版本

指定版本号以匹配现有的 dcc-mcp-core 部署：
...
安装旧版本以回滚。注意 UXP 插件版本必须与 Python 包版本匹配：
...
通过 Creative Cloud Desktop → Plugins → Manage Plugins 移除 UXP 插件，或从 UXP 插件目录中删除 `.ccx` 文件：
